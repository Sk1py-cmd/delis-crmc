...
/**
 * Регрессия: последовательности номеров отставали от содержимого таблиц.
 *
 * Миграция `0001_unique_numbers` выполняет `setval` в момент своего
 * применения — то есть на ещё пустой базе. Данные, залитые уже после неё,
 * занимают номера вида DLS-24000…24067, PO-1200…1267, DLS-1000…1015.
 ...
 * чистой базе → вставка строк → выравнивание счётчиков → проверка,
...
describe.skipIf(!url)("последовательности номеров", () => {
  it("после заполнения данными выдают свободный номер заказа", async () => {
    await withFreshDb("t_seq_order", async (c) => {
      // 68 заказов, начиная с DLS-24000.
      for (let i = 0; i < 68; i++) {
