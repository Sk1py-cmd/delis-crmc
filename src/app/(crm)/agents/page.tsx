...
import { getAgentControlSummary } from "@/server/agentControl";
import { AgentsClient } from "./AgentsClient";
import { AgentControlOverview } from "./AgentControlOverview";
...
export default async function AgentsPage() {
  ...
  const [rows, visits, products, control] = await Promise.all([
    getAgents(),
    getAgentVisits(),
    getProducts(),
  ...
  return (
    <>
      <AgentControlOverview data={control} />
      <AgentsClient
        agents={rows.map((a) => ({
          id: a.id,
          name: a.name,
